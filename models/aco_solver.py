...
class AcoSolver:
    ...
    def combined_tweak_tasks(self, solution, data):
        """
    ...
    def run_algorithm111(self, data, alpha=0.1, iterations=1000, population_size=5):
        ...
        2. Use GRASP-based initializer
        3. For each iteration, build population using:
            * some solutions from generate_solution_aco
            * some solutions from combined_tweak_tasks
        Then update pheromones and track best.
        """
        # 1) init pheromones
        self.init_pheromones(data)

        # 2) GRASP base solution
        base_sol = self.base_solver.generate_initial_solution_grasp(data, p=0.05, max_time=20)
        best_sol = base_sol
        ...
        for it in range(iterations):
            ...
            for _ in range(population_size):
                if random.random() < 0.5:
                    # generate using pheromones
                    sol = self.generate_solution_aco(data)
                    population.append(sol)
                else:
                    # tweak best solution
                    neighbor = self.combined_tweak_tasks(copy.deepcopy(best_sol), data)
